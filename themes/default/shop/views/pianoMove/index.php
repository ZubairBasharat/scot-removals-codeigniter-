...
$selected_products = !empty($storage) ? json_decode($storage->products_list) : array();
// if($last==null || $last==0)
// {
...
<script>
   $(document).ready(function(){
      $(".dropdown_content.select_moving_piano ul li").mouseover(function() {
       var s_attr =$(this).find('button img').attr('src');
       $('.items_icon_shower img').attr('src' , s_attr); 
      });
     "<?php if(!empty($selected_products)) {if($selected_products[0]->type="piano_removals "){ ?>"
      $('.piano_select_btn'+<?= $selected_products[0]->id ?>).click();
   "<?php }} ?>"
   });
</script>
   ...
   <div class="site_banner p-0" style="background:none;">
      <div class="row m-0">
         <div class="col-lg-8 col-md-6 pr-md-4 pl-0 wow slideInLeft pr-0" data-wow-duration="1.5s">
            <div class="bg-white p-xl-5 p-3 bg_drop px-sm-10" id="left-box">
               ...
               <div class="banner_inner_form m-0">
                  <form class="row w-100 m-0">
                     <div class="col-sm-12 pl-0 pr-0">
                        <!-- Dropdown Start -->
                        <div class="w-100 main_drop_aria dop4">
                           <div class="form_input drop_btn" id="select_moving_piano" >
                              <span class="btn_selected_text" data-id="0">What are you moving?</span>
                              <span class="fa fa-angle-down float-right mr-3 toggle_icon"></span>
                           </div>
                           <div class="dropdown_list">
                              <div class="dropdown_content select_moving_piano" >
                                 <div class="row m-0">
                                    <div class="col-md-6 px-0">
                                       <ul class="p-0">
                                          <?php foreach ($products as $p) { ?>
                                          <li>
                                             <button type="button" class="piano_select_btn<?=$p->id?>" onclick="piano_name('<?=$p->name?>','<?=$p->id?>')"> <img src="<?=base_url()?>assets/uploads/thumbs/<?=$p->image?>" class="svg" style="width:30px;height:30px;"><span data-id="<?=$p->id?>"><?=$p->name?></span></button>
                                          </li>
                                          <?php } ?>
                                       </ul>
                                    </div>
                                    <div class="col-6 d-md-block d-none px-0">
                                       <div class="items_icon_shower">
                                          <img class="w-100" src="<?=base_url('assets/uploads/piano-label.jpg')?>">
                                       </div>
                                    </div>
                                 </div>
                              </div>
                     ...
                     <div class="col-lg-6 pr-lg-2 pr-0 pl-0">
                     ...
                     <div class="col-lg-6  pl-0 pl-lg-2 pr-0 mt-1">
                        ...
                        <div class="w-100 main_drop_aria dop4 select_property" id="open_drop_li_click" >
                        ...
                        <div id="inner_checkbox1" class="position-relative lift_availability" style="display:none;">
                     ...
                     <div class="col-lg-6 pl-lg-2 pl-0 pr-0 mt-1">
                        <!-- Inner_Dropdown2 Start -->
                        <div class="w-100 main_drop_aria dop3 select_property" id="open_drop_li_click2">
                        ...
                        <div id="inner_checkbox2" class="position-relative lift_availability" style="display:none;">
                           <label class="container_checkbox">Lift Available
                     ...
                     <div class="instance_btn w-100 position-relative">
                        <button type="button" onclick="getPrices();" class="price_btn border-0 w-100 text-capitalize">
                          view my price
                        </button>
         ...
         <div class="col-lg-4 col-md-6 p-0 ml-auto mt-4 mt-md-0">
            ...
            <!-- <div class="banner_inner_img h-100"> -->
               ...
               <div class="map-container-lg h-lg-350h">
...
<!-- <script src="https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=places&extension=.js&key=AIzaSyDYQxhmQ1_tQluXoWuPiftKMxQsorczNUI"></script> -->
...
// }
   ...
   function getPrices(){
      ...
      // localStorage.setItem("drop",$("#drop").val());
       var id = $("#select_moving_piano span:first-child").attr('data-id');
       ...
       {
         property1 = $("#catagory_inner2 span:first-child").attr('data-slug');
         property2 = $("#catagory_inner3 span:first-child").attr('data-slug');
       }
         function no_item_select(){
            swal(
            'Empty',
      ...
      if(grand_piano==false)
       ...
       $.ajax({
           ...
           dataType: 'json',
           data: {'id': id, 'slug': slug, 'type': "piano_transport"},
           beforeSend: function() {
               $('.button-spinner').show();
                     ...
                     if (status !== 'OK') {
                        ...
                        for (var i = 0; i < originList.length; i++) {
                              ...
                              for (var j = 0; j < results.length; j++) {
                                 ...
                                 //    dataType: 'json',
                                 //    data: {'price': amount, 'type': "piano_transport"},
                                 //    success: function(data) {
                        ...
                        $.ajax({
                              ...
                              dataType: 'json',
                              data: {'slug': slug, 'type': "piano_transport", 'pickup': pickup, 'drop': drop , 'storage_id': "<?= $storage_id ?>", 'price': price, 'km': km, 'total': amount , 'items' : piano},
                              success:function(data)
                              {
   ...
   function piano_name(piano_nam,piano_id)
   {
      if(piano_nam=="Grand Piano")
      {
         $(".select_property").parent().slideUp();
         $(".lift_availability").parent().slideUp();
         $('#drop').parent().parent().removeClass('pr-lg-2');
         property1 = "ground";
         property2 = "ground"; 
         grand_piano = true;
      }
      ...
      {
         $(".select_property").parent().slideDown();
         $('#drop').parent().parent().addClass('pr-lg-2');
         property1 = $("#catagory_inner2 span:first-child").attr('data-slug');
         property2 = $("#catagory_inner3 span:first-child").attr('data-slug');
         if(property1!="ground" && property1!="")
         {
            $("#inner_checkbox1").css('display',"block");
         }
         if(property2!="ground" && property2!="")
         {
      ...
      piano = [];
      piano_obj = {"name":piano_nam , "id":piano_id , "quantity":"1","tab":"null","type":"piano_removals"};
      piano.push(piano_obj);
   }
