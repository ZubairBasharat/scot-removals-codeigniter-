...
$(document).ready(function() {
    ...
    $(document).on('click', '.po-delete11', function(e) {
        e.preventDefault();
        $('.po').popover('hide');
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(data) {
                if (data.error == 1) { location.reload(); } else { location.reload(); }
            },
            error: function(data) { addAlert('Ajax call failed', 'danger'); }
        });
        return false;
...
$(document).ready(function() {
    ...
    $(document).on('click', '.row_price', function(e) {
        e.preventDefault;
        var row = $(this).closest('tr');
        var id = row.attr('id');
        if (row.hasClass('product_link')) {
            $('#myModal').modal({ remote: site.base_url + 'products/quick_update_price/' + id });
            $('#myModal').modal('show');
        }
        return false;
    });
...
function formatDecimalsCurrency(x) {
    d = site.settings.decimals;
    return parseFloat(accounting.formatNumber(x, d, '', '.')).toFixed(d);
}

function formatDecimalsCurrencyModal(x) {
    d = site.settings.decimals;
    value = parseFloat(accounting.formatNumber(x, d, '', '.')).toFixed(d);
    return '<div class="text-center"><span class="row_price label label-warning">' + value + '</span></div>';
}
...
$(document).ready(function() {
    ...
    $('body').on('click', '.product_link td:not(:first-child, :nth-child(6), :last-child)', function() {
    ...
    $('body').on('click', '.floor_link td:not(:first-child, :nth-child(6), :last-child)', function() {
        $('#myModal').modal({ remote: site.base_url + 'floors/modal_view/' + $(this).parent('.floor_link').attr('id') });
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'products/view/' + $(this).parent('.product_link').attr('id');
    });
    ...
    $('body').on('click', '.invoice_link td:not(:first-child, :nth-child(5), :nth-last-child(2), :last-child)', function() {
