$(document).ready(function(e) {
    ...
    $('#slshipping').focus(function() {
        ...
        $('#gtotal').text(formatDecimalsCurrency(gtotal));
        //$('#tship').text(formatDecimalsCurrency(shipping));
    });
    ...
    * Sell Gift Card modal
    ...
    $(document).on('click', '#addItemManually', function(e) {
        ...
        if (mcode && mname && mqty && unit_price) {
            slitems[mid] = { "id": mid, "item_id": mid, "label": mname + ' (' + mcode + ')', "row": { "id": mid, "code": mcode, "name": mname, "quantity": mqty, "base_quantity": mqty, "price": unit_price, "unit_price": unit_price, "real_unit_price": unit_price, "unit": munit, "qty": mqty, "type": "manual", "discount": mdiscount, "serial": "", "option": "" }, 'units': false, "options": false };
            localStorage.setItem('slitems', JSON.stringify(slitems));
            loadItems();
        }
        $('#mModal').modal('hide');
...
function loadItems() {
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            unit_price = formatDecimalsCurrency(unit_price - item_discount);
            item_price = formatDecimalsCurrency(unit_price);
            unit_price = formatDecimalsCurrency(unit_price + item_discount);
            var row_no = item.id;
            var newTr = $('<tr id="row_' + row_no + '" class="row_' + item_id + '" data-item-id="' + item_id + '"></tr>');
            tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><span class="sname" id="name_' + row_no + '">' + item_code + ' - ' + item_name + '</span> <i class="pull-right fa fa-edit tip pointer edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';

            tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatDecimalsCurrency(item_price) + '</span></td>';
            ...
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatDecimalsCurrency(((parseFloat(item_price)) * parseFloat(item_qty))) + '</span></td>';
        ...
        tfoot += '<th class="text-right">' + formatDecimalsCurrency(total) + '</th><th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
        ...
        $('#total').text(formatDecimalsCurrency(total));
        $('#titems').text((an - 1) + ' (' + formatQty(parseFloat(count) - 1) + ')');
        $('#total_items').val((parseFloat(count) - 1));
        
        $('#tds').text(formatDecimalsCurrency(order_discount));
