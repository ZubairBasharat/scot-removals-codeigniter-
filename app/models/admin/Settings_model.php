...
class Settings_model extends CI_Model
{
    ...
    function updateCategoryTranslation($postdata, $id) {
		foreach($postdata as $lang => $val){
		     if(array_filter($val)){
                $name = $val['name'];
                $description = $val['description'];
                $transAvailable = $this->getCategoryTranslation($lang, $id);
                if(empty($transAvailable)){
					...
					$this->db->insert('categories_translation', $data);
				}else{
					$data = array(
                        'trans_name' => $name,
                        'trans_description' => $description
					...
					$this->db->update('categories_translation', $data);
				}
			}
		}
    }
    
    function getCategoryTranslation($lang, $id){
		$this->db->where('trans_lang', $lang);
        $this->db->where('category_id', $id);
		return $this->db->get('categories_translation')->result();
    }
