...
class MY_Shop_Controller extends CI_Controller {
    ...
    function page_construct($page, $data = array()) {
        if (SHOP) {
            ...
            // $data['storage'] = $this->shop_model->get_storage($this->input->ip_address());
            if(isset($_COOKIE["scot_cookie_id"])) {
                $data['storage'] = $this->shop_model->get_storage($_COOKIE["scot_cookie_id"]);
            }else{
                $data['storage'] = array();
            }
            // print_r($data['storage']);exit;
