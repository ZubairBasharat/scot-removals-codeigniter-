...
class Sma{
    ...
    public function formatMoney($number, $symbol = false)
    {
        ...
        $this->formatDecimal($number, $decimals) .
    ...
    public function qrcode($type = 'text', $text = 'https://adroitlight.com', $size = 2, $level = 'H', $sq = null)
    {
    ...
    public function send_fcm_notif($fcm_token, $title, $body){  
        ...
        $headers = array(
        'Authorization: key=AAAAdMF-hb8:APA91bHHioaTseDkFj2NJDuyJlZt0n2NoxEO_clIu0q2zOAA7LXOTtLaIonwnfh_0DeiSxG_-u1ZeXLmf9duyKaxj_sGtlTm_zzFVNTP7rU0zzW9utSlY3XCIZvMclUTh7-qSKEsJZCg' ,
    ...
    public function getFcmTokenByID($customer_id) {
        $this->db->select("{$this->db->dbprefix('companies')}.fcm_token")
        ->where('companies.id', $customer_id);
        $q = $this->db->get('companies', 1);
        if ($q->num_rows() > 0) {
            return $q->row()->fcm_token;
        }else{
            return false;
        }
    }
