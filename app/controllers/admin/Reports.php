...
class Reports extends MY_Controller
{
    ...
    function getSalesReport($pdf = NULL, $xls = NULL){
        ...
        if ($pdf || $xls) {
            ...
            $si = "( SELECT sale_id, product_id, GROUP_CONCAT(CONCAT({$this->db->dbprefix('sale_items')}.product_name, '__', {$this->db->dbprefix('sale_items')}.quantity) SEPARATOR '___') as item_nane from {$this->db->dbprefix('sale_items')} ";
            
            $si .= " GROUP BY {$this->db->dbprefix('sale_items')}.sale_id ) FSI";
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, FSI.item_nane as iname, grand_total, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
