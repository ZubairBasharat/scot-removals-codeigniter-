...
class Auth extends MY_Controller{
    ...
    function getUsers(){
        ...
        $this->datatables
            ->select($this->db->dbprefix('users').".id as id, name, email, active")
            ->from("users")
            ->group_by('users.id')
    ...
    function create_user(){
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)) {
            ...
            $meta = array('page_title' => lang('users'), 'bc' => $bc);
            $count = $this->auth_model->getUserCount();
            if($count <= 50){
                $this->page_construct('auth/create_user', $meta, $this->data);
            }else{
                $this->session->set_flashdata('error', "You cannot add more than 50 users please contact to administrator!");
